...
describe("Funnel Navigation", () => {
    test("navigation with onSubmitActivateStep action", async () => {
        ...
        expect(submission1.completedStepIds).toEqual(["step1", "step2"]);

        // Going back should bring us to step 2.
        submission1.activatePreviousStep();
        expect(submission1.getActiveStep().id).toBe("step2");
        expect(submission1.isSuccessStep()).toBe(false);
        expect(submission1.completedStepIds).toEqual(["step1"]);

        // Going back should bring us to step 1.
        submission1.activatePreviousStep();
        expect(submission1.getActiveStep().id).toBe("step1");
        expect(submission1.isSuccessStep()).toBe(false);
        expect(submission1.isFirstStep()).toBe(true);
        expect(submission1.completedStepIds).toEqual([]);

        // No skipping of steps here.
        ...
        // Going back should bring us to step 2.
        submission2.activatePreviousStep();
        expect(submission2.getActiveStep().id).toBe("step2");
        expect(submission2.isSuccessStep()).toBe(false);
        expect(submission2.isFirstStep()).toBe(false);
        expect(submission2.completedStepIds).toEqual(["step1"]);

        // Going back should bring us to step 1.
        submission2.activatePreviousStep();
        expect(submission2.getActiveStep().id).toBe("step1");
        expect(submission2.isSuccessStep()).toBe(false);
        expect(submission2.isFirstStep()).toBe(true);
        expect(submission2.completedStepIds).toEqual([]);
    });
});

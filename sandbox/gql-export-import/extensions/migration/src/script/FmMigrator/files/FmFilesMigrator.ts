...
export class FmFilesMigrator {
    ...
    async run() {
        ...
        do {
            ...
            for (const sourceFile of sourceFilesList.data) {
                // 1. Get presigned post payload.
                console.log(`Migrating file ${sourceFile.name}...`);

                let downloadedFile: File | null = null;
                try {
                    downloadedFile = await downloadFile(sourceFile);
                } catch (error) {
                    console.error(
                        `Failed to download file "${sourceFile.name}". Error: ${error.message}`
                    );
                    continue;
                }
                ...
                if (error) {
                    console.log(
                        `Failed to migrate file "${sourceFile.name}". Error:`,

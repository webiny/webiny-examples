export const containerElementExistsMock = {
    id: "6821d94b28eb04960a6ba289#0001",
    content: {
        id: "1k547ym",
        type: "document",
        data: {},
        elements: [
            {
                id: "a5f7ifc",
                type: "block",
                data: {
                    settings: {
                        width: {
                            desktop: {
                                value: "100%"
                            }
                        },
                        margin: {
                            desktop: {
                                top: "0px",
                                right: "0px",
                                bottom: "0px",
                                left: "0px",
                                advanced: true
                            }
                        },
                        padding: {
                            desktop: {
                                all: "10px"
                            }
                        },
                        horizontalAlignFlex: {
                            desktop: "center"
                        },
                        verticalAlign: {
                            desktop: "flex-start"
                        }
                    }
                },
                elements: [
                    {
                        id: "fub-container-123",
                        type: "fub-container-123",
                        data: {
                            settings: {},
                            fields: [
                                {
                                    id: "fm1dkir",
                                    fieldId: "firstName",
                                    stepId: "1kb7kgk",
                                    type: "text",
                                    label: "firstName",
                                    helpText: "",
                                    validators: [],
                                    value: {
                                        type: "string",
                                        value: "",
                                        array: false
                                    },
                                    extra: {
                                        placeholderText: ""
                                    }
                                },
                                {
                                    id: "jhos45r",
                                    fieldId: "lastName",
                                    stepId: "1kb7kgk",
                                    type: "text",
                                    label: "lastName",
                                    helpText: "",
                                    validators: [],
                                    value: {
                                        type: "string",
                                        value: "",
                                        array: false
                                    },
                                    extra: {
                                        placeholderText: ""
                                    }
                                }
                            ],
                            steps: [
                                {
                                    id: "1kb7kgk",
                                    title: "New page"
                                },
                                {
                                    id: "success",
                                    title: "Success page"
                                }
                            ],
                            conditionRules: []
                        },
                        elements: [
                            {
                                id: "e2tj2b6",
                                type: "fub-step",
                                data: {
                                    settings: {},
                                    stepId: "1kb7kgk"
                                },
                                elements: [
                                    {
                                        id: "lkwals5",
                                        type: "grid",
                                        data: {
                                            isFunnelStepGrid: true,
                                            settings: {
                                                width: {
                                                    desktop: {
                                                        value: "1100px"
                                                    }
                                                },
                                                margin: {
                                                    desktop: {
                                                        top: "0px",
                                                        right: "0px",
                                                        bottom: "0px",
                                                        left: "0px",
                                                        advanced: true
                                                    }
                                                },
                                                padding: {
                                                    desktop: {
                                                        all: "10px"
                                                    }
                                                },
                                                grid: {
                                                    cellsType: "12"
                                                },
                                                gridSettings: {
                                                    desktop: {
                                                        flexDirection: "row"
                                                    },
                                                    "mobile-landscape": {
                                                        flexDirection: "column"
                                                    }
                                                },
                                                horizontalAlignFlex: {
                                                    desktop: "flex-start"
                                                },
                                                verticalAlign: {
                                                    desktop: "flex-start"
                                                }
                                            }
                                        },
                                        elements: [
                                            {
                                                id: "nz7afpf",
                                                type: "cell",
                                                data: {
                                                    settings: {
                                                        margin: {
                                                            desktop: {
                                                                top: "0px",
                                                                right: "0px",
                                                                bottom: "0px",
                                                                left: "0px",
                                                                advanced: true
                                                            }
                                                        },
                                                        padding: {
                                                            desktop: {
                                                                all: "0px"
                                                            }
                                                        },
                                                        grid: {
                                                            size: 12
                                                        },
                                                        horizontalAlignFlex: {
                                                            desktop: "flex-start"
                                                        }
                                                    }
                                                },
                                                elements: [
                                                    {
                                                        id: "V4ehNeekPB",
                                                        type: "fub-field-text",
                                                        data: {
                                                            id: "fm1dkir",
                                                            fieldId: "firstName",
                                                            stepId: "1kb7kgk",
                                                            type: "text",
                                                            label: "firstName",
                                                            helpText: "",
                                                            validators: [],
                                                            value: {
                                                                type: "string",
                                                                value: "",
                                                                array: false
                                                            },
                                                            extra: {
                                                                placeholderText: ""
                                                            }
                                                        },
                                                        elements: [],
                                                        path: [
                                                            "1k547ym",
                                                            "fub-container",
                                                            "e2tj2b6",
                                                            "lkwals5",
                                                            "nz7afpf"
                                                        ]
                                                    },
                                                    {
                                                        id: "3ypzgZAu8o",
                                                        type: "fub-field-text",
                                                        data: {
                                                            id: "jhos45r",
                                                            fieldId: "lastName",
                                                            stepId: "1kb7kgk",
                                                            type: "text",
                                                            label: "lastName",
                                                            helpText: "",
                                                            validators: [],
                                                            value: {
                                                                type: "string",
                                                                value: "",
                                                                array: false
                                                            },
                                                            extra: {
                                                                placeholderText: ""
                                                            }
                                                        },
                                                        elements: [],
                                                        path: [
                                                            "1k547ym",
                                                            "fub-container",
                                                            "e2tj2b6",
                                                            "lkwals5"
                                                        ]
                                                    }
                                                ],
                                                path: [
                                                    "1k547ym",
                                                    "fub-container",
                                                    "e2tj2b6",
                                                    "lkwals5"
                                                ]
                                            }
                                        ],
                                        path: ["1k547ym", "fub-container", "e2tj2b6"]
                                    }
                                ],
                                path: ["1k547ym", "fub-container"]
                            },
                            {
                                id: "znvruk7",
                                type: "fub-step",
                                data: {
                                    settings: {},
                                    stepId: "success"
                                },
                                elements: [
                                    {
                                        id: "qvpq482",
                                        type: "grid",
                                        data: {
                                            isFunnelStepGrid: true,
                                            settings: {
                                                width: {
                                                    desktop: {
                                                        value: "1100px"
                                                    }
                                                },
                                                margin: {
                                                    desktop: {
                                                        top: "0px",
                                                        right: "0px",
                                                        bottom: "0px",
                                                        left: "0px",
                                                        advanced: true
                                                    }
                                                },
                                                padding: {
                                                    desktop: {
                                                        all: "30px"
                                                    }
                                                },
                                                grid: {
                                                    cellsType: "12"
                                                },
                                                gridSettings: {
                                                    desktop: {
                                                        flexDirection: "row"
                                                    },
                                                    "mobile-landscape": {
                                                        flexDirection: "column"
                                                    }
                                                },
                                                horizontalAlignFlex: {
                                                    desktop: "flex-start"
                                                },
                                                verticalAlign: {
                                                    desktop: "flex-start"
                                                }
                                            }
                                        },
                                        elements: [
                                            {
                                                id: "wgbjp26",
                                                type: "cell",
                                                data: {
                                                    settings: {
                                                        margin: {
                                                            desktop: {
                                                                top: "0px",
                                                                right: "0px",
                                                                bottom: "0px",
                                                                left: "0px",
                                                                advanced: true
                                                            }
                                                        },
                                                        padding: {
                                                            desktop: {
                                                                all: "0px"
                                                            }
                                                        },
                                                        grid: {
                                                            size: 12
                                                        },
                                                        horizontalAlignFlex: {
                                                            desktop: "flex-start"
                                                        }
                                                    }
                                                },
                                                elements: [
                                                    {
                                                        id: "qpyfp7p",
                                                        type: "icon",
                                                        data: {
                                                            icon: {
                                                                icon: {
                                                                    type: "icon",
                                                                    name: "fa6_solid_check",
                                                                    value: '<path fill="currentColor" d="M438.6 105.4c12.5 12.5 12.5 32.8 0 45.3l-256 256c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0L160 338.7l233.4-233.3c12.5-12.5 32.8-12.5 45.3 0z"/>',
                                                                    category: "Interfaces",
                                                                    width: 448,
                                                                    color: "#7ed321"
                                                                },
                                                                markup: '<svg width="50" height="50" viewBox="0 0 448 512" color="#7ed321" style="vertical-align: middle;"><path fill="currentColor" d="M438.6 105.4c12.5 12.5 12.5 32.8 0 45.3l-256 256c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0L160 338.7l233.4-233.3c12.5-12.5 32.8-12.5 45.3 0z"></path></svg>',
                                                                width: 50
                                                            },
                                                            settings: {
                                                                horizontalAlign: "center",
                                                                margin: {
                                                                    desktop: {
                                                                        all: "0px"
                                                                    }
                                                                },
                                                                padding: {
                                                                    desktop: {
                                                                        all: "0px"
                                                                    }
                                                                }
                                                            }
                                                        },
                                                        elements: [],
                                                        path: [
                                                            "1k547ym",
                                                            "fub-container",
                                                            "znvruk7",
                                                            "qvpq482",
                                                            "wgbjp26"
                                                        ]
                                                    },
                                                    {
                                                        id: "3m1u5ey",
                                                        type: "heading",
                                                        data: {
                                                            text: {
                                                                desktop: {
                                                                    type: "heading",
                                                                    alignment: "left",
                                                                    tag: "h1"
                                                                },
                                                                data: {
                                                                    text: '{"root":{"children":[{"children":[{"detail":0,"format":0,"mode":"normal","style":"","text":"Success!","type":"text","version":1}],"direction":"ltr","format":"center","indent":0,"type":"heading-element","version":1,"tag":"h1","styles":[{"styleId":"heading1","type":"typography"}]}],"direction":"ltr","format":"","indent":0,"type":"root","version":1}}'
                                                                }
                                                            },
                                                            settings: {
                                                                margin: {
                                                                    desktop: {
                                                                        all: "0px"
                                                                    }
                                                                },
                                                                padding: {
                                                                    desktop: {
                                                                        all: "0px"
                                                                    }
                                                                }
                                                            }
                                                        },
                                                        elements: [],
                                                        path: [
                                                            "1k547ym",
                                                            "fub-container",
                                                            "znvruk7",
                                                            "qvpq482",
                                                            "wgbjp26"
                                                        ]
                                                    },
                                                    {
                                                        id: "nkxcp74",
                                                        type: "paragraph",
                                                        data: {
                                                            text: {
                                                                data: {
                                                                    text: '{"root":{"children":[{"children":[{"detail":0,"format":0,"mode":"normal","style":"","text":"Thank you for your submission!","type":"text","version":1}],"direction":"ltr","format":"center","indent":0,"type":"paragraph-element","version":1,"textFormat":0,"styles":[{"styleId":"paragraph1","type":"typography"}]}],"direction":"ltr","format":"","indent":0,"type":"root","version":1}}'
                                                                }
                                                            },
                                                            settings: {
                                                                margin: {
                                                                    desktop: {
                                                                        all: "0px"
                                                                    }
                                                                },
                                                                padding: {
                                                                    desktop: {
                                                                        all: "0px"
                                                                    }
                                                                }
                                                            }
                                                        },
                                                        elements: [],
                                                        path: [
                                                            "1k547ym",
                                                            "fub-container",
                                                            "znvruk7",
                                                            "qvpq482",
                                                            "wgbjp26"
                                                        ]
                                                    }
                                                ],
                                                path: [
                                                    "1k547ym",
                                                    "fub-container",
                                                    "znvruk7",
                                                    "qvpq482"
                                                ]
                                            }
                                        ],
                                        path: ["1k547ym", "fub-container", "znvruk7"]
                                    }
                                ],
                                path: ["1k547ym", "fub-container"]
                            }
                        ],
                        path: ["1k547ym"]
                    }
                ],
                path: ["1k547ym"]
            }
        ],
        path: []
    },
    title: "Untitled",
    path: "/untitled-makzqi1n",
    status: "draft",
    savedOn: "2025-05-12T11:23:09.827Z",
    dataBindings: [],
    dataSources: [],
    __typename: "PbPage"
};

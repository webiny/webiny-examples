...
// Content Platform plugins
import { createContentPlatform } from "@demo/api";
...
export const handler = createHandler({
    plugins: [
        ...
        createContentPlatform({
            ...
            modules: [
                {
                    contentModelId: "article",
                    pageTemplateSlug: "article",
                    homepage: "/articles",
                    articlePath: entry => `/articles/${entry.entryId}`,
                    pageTags: ["article"],
                    defaultPages: ["/articles"]
                },
                {
                    contentModelId: "news",
                    pageTemplateSlug: "news",
                    homepage: "/news",
                    articlePath: entry => `/news/${entry.entryId}`,
                    pageTags: ["news"],
                    defaultPages: ["/news"]
                },
                {
                    contentModelId: "staticPage",
                    pageTemplateSlug: "static-page",
                    // Static pages need to have a `path` field, e.g.: /terms-of-service
                    articlePath: entry => entry.values["path"] || entry.entryId,
                    pageTags: ["static"]
                }
            ]
        })

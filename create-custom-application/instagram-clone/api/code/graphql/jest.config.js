...
/**
 * `getStackOutput` retrieves all values that were exported from `instagram-clone/api/pulumi/dev/index.ts`.
...
if (TEST_TYPE !== "unit") {
    log.info(
        `${log.info.hl(
            "instagram-clone/api/code/graphql"
        )}: Assigning environment variables...`
    );
    const stackOutput = getStackOutput({
        folder: "instagram-clone/api",

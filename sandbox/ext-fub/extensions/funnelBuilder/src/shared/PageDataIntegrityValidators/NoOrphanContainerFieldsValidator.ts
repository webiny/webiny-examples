...
export class NoOrphanContainerFieldsValidator {
    static id = "NoOrphanContainerFieldsValidator";
    ...
    static validate(page: PbPage) {
        return withContainer(page, containerElement => {
            ...
            const orphanFields = fieldIdsFoundInContainerElement.filter(
                fieldId => !fieldIdsFoundInFieldElements.includes(fieldId)
            );

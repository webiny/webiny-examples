...
export const utils: ApplicationUtils = {
     ...
     */
    db: () => ({
        table: process.env.DB_TABLE_CAR_MANUFACTURER,
        keys: [
            {
                primary: true,
                unique: true,
    ...
    es(context) {
        ...
        return {
            index: `${tenant.id}-car-manufacturer`
        };
    },
    /**
    ...
    createPk: (context, id) => {
        ...
        return `T#${tenant.id}#carManufacturer#${id}`;
        /**
         * If you want to use different locales per carManufacturer, uncomment code below.
        ...
        // }
        // return `T#${tenant.id}#L#${locale.code}#carManufacturer#${id}`;
    }
};
